import random

def description():
    print("Welcome to 21 Number Game!\n" \
    "The objective of this game is to not be the one to call 21.\n" \
    "The game is played between you and the computer.\n" \
    "You can call 1 to 3 consecutive numbers in your turn.\n" \
    "Start from 1 and continue upward.\n" \
    "If you are the one to call 21, you lose! Else, you win!\n" \
    "Good luck and may the odds be ever in your favor!\n")

    player_turn = False
    computer_turn = False

    starting_player = input("Do you want to go first or second? [F/S]: ")

    match starting_player.upper():
        case "F":
            player_turn = True
        case "S":
            computer_turn = True
        case _:
            print("You are disqualified!")
            return player_turn, computer_turn
        
    return player_turn, computer_turn

def player_loop(called_nums, last_num):
    end_reached = False
    player_turn = False
    computer_turn = False

    try:
        player_nums = [int(x) for x in input("Call 1 to 3 consecutive numbers: ").split()]
    except ValueError:
        print("Enter integers only.")
        player_turn = True
        return player_turn, computer_turn, called_nums, end_reached

    if not player_nums:
        print("You must enter at least one number!")
        player_turn = True
        return player_turn, computer_turn, called_nums, end_reached
    elif len(player_nums) > 3:
        print("You are disqualified!")
        end_reached = True
        return player_turn, computer_turn, called_nums, end_reached
    else:
        if (last_num + 1 >= 21):
            player_turn = True
            end_reached = True
            return player_turn, computer_turn, called_nums, end_reached

        num1 = player_nums[0]
        if num1 != last_num + 1 or num1 >= 21:
            player_turn = True
            end_reached = True
            return player_turn, computer_turn, called_nums, end_reached
        else:
            called_nums.extend([num1])
        if len(player_nums) > 1:
            num2 = player_nums[1]
            if num2 != num1 + 1 or num2 >= 21:
                player_turn = True
                end_reached = True
                return player_turn, computer_turn, called_nums, end_reached
            else:
                called_nums.extend([num2])
        else:
            num2 = None
        if len(player_nums) > 2:
            num3 = player_nums[2]
            if num3 != num2 + 1 or num3 >= 21:
                player_turn = True
                end_reached = True
                return player_turn, computer_turn, called_nums, end_reached
            else:
                called_nums.extend([num3])
        else:
            num3 = None

    player_turn = False
    computer_turn = True

    return player_turn, computer_turn, called_nums, end_reached

def computer_loop(called_nums, last_num):
    end_reached = False
    player_turn = False
    computer_turn = False
    next_num = last_num + 1
    number_of_nums = random.randint(1,3)

    if (next_num >= 21):
        player_turn = False
        computer_turn = True
        end_reached = True
        return computer_turn, player_turn, called_nums, end_reached
    elif (next_num + 1 == 21):
        number_of_nums = 1
    elif (next_num + 2 == 21):
        number_of_nums = 2

    if (number_of_nums == 1):
        called_nums.extend([next_num])
    elif (number_of_nums == 2):
        called_nums.extend([next_num, next_num + 1])
    elif (number_of_nums == 3):
        called_nums.extend([next_num, next_num + 1, next_num + 2])

    computer_turn = False
    player_turn = True

    return computer_turn, player_turn, called_nums, end_reached

def end_condition(player_turn, computer_turn):
    if (player_turn):
        print("\nYou lose!")
    elif (computer_turn):
        print("\nYou win!")
    else:
        return

    return

def game_loop(player_turn, computer_turn):
    called_nums = []  # start with zero to get last_num
    last_num = called_nums[len(called_nums)-1] if called_nums else 0 # check for consecutiveness

    while (last_num < 21):
        last_num = called_nums[len(called_nums)-1] if called_nums else 0

        if (player_turn):
            player_turn, computer_turn, called_nums, end_reached = player_loop(called_nums, last_num)
            if (end_reached):
                end_condition(player_turn, computer_turn)
                break
            print(f"P1 called: {called_nums}")
        elif (computer_turn):
            computer_turn, player_turn, called_nums, end_reached = computer_loop(called_nums, last_num)
            if (end_reached):
                end_condition(player_turn, computer_turn)
                break
            print(f"P2 called: {called_nums}\n")
        elif (not player_turn and not computer_turn):
            break

def main():
    player_turn, computer_turn = description()
    game_loop(player_turn, computer_turn)

if __name__ == "__main__":
    main()